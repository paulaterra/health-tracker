...
const SUGGESTIONS=[
  "Quan em fa més mal l'esquena?",
  "Què tenen en comú els dies pitjors?",
  "El dolor té relació amb el cicle?",
  "Quan tinc més diarrea?",
  "Com ha canviat el son aquest mes?",
  "Quins factors coincideixen amb dies millors?",
];
...
export async function renderAssistant(container){
  container.innerHTML=`<div class="view-header"><span class="view-eyebrow">IA local</span><h1 class="view-title">Pregunta a Paula Tracker</h1><p class="view-sub">Respostes generades exclusivament a partir del teu historial registrat.</p></div><div class="card"><p class="ledger-empty">Preparant les dades…</p></div>`;
  const [matrix,intel]=await Promise.all([buildDailyMatrix(),generateIntelligence()]);
  container.innerHTML=`
    <div class="view-header"><span class="view-eyebrow">IA local</span><h1 class="view-title">Pregunta a Paula Tracker</h1><p class="view-sub">Respostes generades exclusivament a partir del teu historial registrat.</p></div>
    <section class="card assistant-query-card">
      <label class="field-label" for="health-question">Què vols saber?</label>
      <div class="assistant-query-row"><input id="health-question" type="text" placeholder="Per exemple: quan em fa més mal l'esquena?"><button id="ask-health" class="btn btn-primary" type="button">Analitza</button></div>
